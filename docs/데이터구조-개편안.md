...
### 4-5. `reminder_events`
- 역할: 실제 티켓이 `IN_PROGRESS`로 진입한 뒤 생성되는 알림 이벤트
- 컬럼
  - `id`
  - `rule_id`
  - `task_ticket_id`
...
- 알림 규칙은 템플릿에 매달린다.
- 티켓은 생성 시점의 `reminder_rule_id`를 스냅샷으로 가진다.
- 실제 알림 이벤트는 `task_ticket`이 `IN_PROGRESS`로 진입하는 시점에 생성한다.
- 알림은 `delay_minutes + repeat_interval_minutes` 기준으로 최대 `max_alert_count`회까지 한 번에 예약한다.
- `reminder_events`는 현재 살아 있는 예약 묶음만 저장하고, 종료 시 row를 삭제한다.
- 알림이 연결된 티켓 상태 의미
  - `PENDING`: 아직 시작 전
...
- 기록 탭: 날짜 범위 내 `task_tickets` 기준 조회
- 루틴 탭: 템플릿 + 반복 규칙 관리 UI로 역할 변경
- 알림 설정: 루틴 폼 안의 접히는 상세 설정으로 이동
